    ...
    while not pq.is_empty():
        ...
        for direction in [
            ...
            if 0 <= (current_cell[2] + theta_offset) < 360:
                neighbour = (current_cell[0] + row_offset, current_cell[1] + col_offset, current_cell[2] + theta_offset)
            elif current_cell[2] + theta_offset > 360:
                neighbour = (current_cell[0] + row_offset, current_cell[1] + col_offset, (current_cell[2] + theta_offset) - 360)
            elif current_cell[2] + theta_offset == 360:
                neighbour = (current_cell[0] + row_offset, current_cell[1] + col_offset, 0)
            elif (current_cell[2] + theta_offset) < 0 :
                neighbour = (current_cell[0] + row_offset, current_cell[1] + col_offset, 360 + (current_cell[2] + theta_offset))
            # Xi,Yi,Thetai,UL,UR

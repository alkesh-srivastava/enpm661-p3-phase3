...
def obstacle(x, y, clearance=0):  # This function definition inspects whether a coordinate point x
    ...
    if 3.75 - clearance <= x <= 6.25 + clearance and 4.25 - clearance <= y <= 5.75 + clearance:
        inside_C2 = True
...
def move_possible(maze, pos, clearence):
    i, j, k, l, m, n = pos
    num_rows = len(maze)
    num_cols = len(maze[0])
    return 0 <= i < num_cols and 0 <= j < num_rows and not (obstacle(i, j, clearence))
...
def in_gvalues(neighbour, g_values):
    return False
